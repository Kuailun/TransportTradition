...
class AMAP_User:
    ...
    def _AnalyzeData(self, content_walk, content_bike, content_bus, content_car, p_data):
        ...
        try:
            # 分析走路
            p_data[20] = int(content_walk['route']['paths'][0]['distance'])
            p_data[21] = int(content_walk['route']['paths'][0]['duration'])

            # 分析骑车
            p_data[18] = int(content_bike['data']['paths'][0]['distance'])
            p_data[19] = int(content_bike['data']['paths'][0]['duration'])

            # 分析开车
            p_data[10] = int(content_car['route']['paths'][0]['distance'])
            p_data[11] = int(content_car['route']['paths'][0]['duration'])
            ...
            # 复制到数据体里
            p_data[12] = bus_distance
            p_data[13] = bus_duration
            p_data[14] = subway_distance
            p_data[15] = subway_duration
            p_data[16] = walk_distance
            p_data[17] = walk_duration
